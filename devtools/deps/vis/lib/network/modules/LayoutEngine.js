/// BareSpecifier=vis/lib/network/modules/LayoutEngine
'use strict';
/**
...
class LayoutEngine {
  ...
  _condenseHierarchy() {
    ...
    let centerAllParents = () => {
      for (let nodeId in this.body.nodes) {
        if (this.body.nodes.hasOwnProperty(nodeId)) this._centerParent(this.body.nodes[nodeId]);
      }
    };
  ...
  _determineLevelsCustomCallback() {
    ...
    let levelByDirection = (nodeA, nodeB, edge) => {
      ...
      let diff = customCallback(NetworkUtil.cloneOptions(nodeA, 'node'), NetworkUtil.cloneOptions(nodeB, 'node'), NetworkUtil.cloneOptions(edge, 'edge'));
