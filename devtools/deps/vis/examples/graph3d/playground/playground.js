/// BareSpecifier=vis/examples/graph3d/playground/playground
...
function trim(text) {
  ...
  while (text.length && text.charAt(text.length - 1) == ' ') text = text.substr(0, text.length - 1);
...
function drawGooglespreadsheet() {
  ...
  drawGraph = function (response) {
    ...
    if (response.isError()) {
      ...
      document.getElementById('graph').innerHTML = "<span style='color: red; font-weight: bold;'>" + error + "</span>";;
    }
