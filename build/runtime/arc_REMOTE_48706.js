...
export class Arc {
    ...
    createInnerArc(transformationParticle) {
        const recipe = this.recipes.find(r => r.particles.includes(transformationParticle));
        // We can allow multiple inner arcs if we want to. Waiting for a good use case.
        assert(!recipe.innerArcs.has(transformationParticle), 'Only one inner arc is allowed per particle');
        const id = this.generateID('inner').toString();
        const arc = new Arc({
            id,
            pecFactory: this.pecFactory,
            slotComposer: this.pec.slotComposer,
            loader: this._loader,
            context: new Manifest({ id }),
            innerArc: true, speculative: this.isSpeculative
        });
        recipe.innerArcs.set(transformationParticle, arc);
        return arc;
    }
    getInnerArc(particle) {
        return this.recipes.find(r => r.particles.includes(particle)).innerArcs.get(particle);
    }
    get innerArcs() {
        return [].concat(...this.recipes.map(r => [...r.innerArcs.values()]));
    }
    get loader() {
        return this._loader;
    }
    ...
    async _waitForIdle() {
        while (true) {
            ...
            // If no new inner arcs appeared and this.pec had exactly 2 messages,
            // one requesting the idle status, and one answering it, then we're idle.
...
${this.activeRecipe.toString()}`;
    ...
    async instantiate(recipe) {
        ...
        this._recipes.push({ particles, handles, slots, innerArcs: new Map(), patterns: recipe.patterns });
        ...
        if (!this.isSpeculative && !this.isInnerArc) {
            // Note: callbacks not triggered for inner or speculative arcs.
...
//# sourceMappingURL=arc_REMOTE_48706.js.map