...
${particleStr1}
  ...
  modality dom`;
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.slots, 2);
        assert.equal(recipe.particles.find(p => p.name === 'ParticleB').consumedSlotConnections['slotB1'].providedSlots['slotB2'], recipe.particles.find(p => p.name === 'ParticleA').consumedSlotConnections['slotA'].targetSlot);
        recipe.normalize();
    ...
    `);
        ...
        const recipe = manifest.recipes[0];
        assert.lengthOf(recipe.handles, 2);
        assert.equal(recipe.particles.find(p => p.name === 'ParticleA').connections['slotA'].handle, recipe.particles.find(p => p.name === 'ParticleB').connections['slotB2'].handle);
        recipe.normalize();
        assert.isTrue(recipe.isResolved());
    });
