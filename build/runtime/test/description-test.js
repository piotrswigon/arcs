...
${recipeManifest}
    `));
            ...
            description = await test.verifySuggestion({ arc }, 'Read from my-in-foo (foo-name) and populate my-out-foos (foo-1 plus 2 other items).');
            assert.equal(description.getHandleDescription(ifooHandle), 'my-in-foo');
            assert.equal(description.getHandleDescription(ofoosHandle), 'my-out-foos');
        });
    });
    tests.forEach((test) => {
        ...
        recipe
          ...
          A
            ...
            assert.isTrue(recipe.isResolved());
            const arc = createTestArc(recipe, manifest);
            const foosStore = await arc.createStore(fooType.bigCollectionOf(), undefined, 'test:1');
            const fooStore = await arc.createStore(fooType, undefined, 'test:2');
            ...
            await test.verifySuggestion({ arc }, 'Read from my-in-foos (collection of items like foo-1) and write my-out-foo.');
        });
    });
    tests.forEach((test) => {
...
${recipeManifest}
    `));
            ...
            foosStore.store({ id: 2, rawData: { name: 'foo-1', fooValue: 'foo-value-1' } }, ['key2']);
            foosStore.store({ id: 3, rawData: { name: 'foo-2', fooValue: 'foo-value-2' } }, ['key3']);
            description = await test.verifySuggestion({ arc }, 'Read from my-in-foo (foo-name) and populate The Foos from my-in-foo (foo-1, foo-2).');
            assert.equal(description.getHandleDescription(ifooHandle), 'my-in-foo');
            assert.equal(description.getHandleDescription(ofoosHandle), 'The Foos from my-in-foo');
        });
    });
...
${recipeManifest}
    `));
            ...
            foosStore.store({ id: 2, rawData: { name: 'foo-1', fooValue: 'foo-value-1' } }, ['key2']);
            foosStore.store({ id: 3, rawData: { name: 'foo-2', fooValue: 'foo-value-2' } }, ['key3']);
            description = await test.verifySuggestion({ arc }, 'Read from foo-name and populate The Foos from foo-name (foo-1, foo-2).');
            assert.equal(description.getHandleDescription(ifooHandle), 'foo');
            assert.equal(description.getHandleDescription(ofoosHandle), 'The Foos from foo');
        });
    });
...
${recipeManifest}
    `));
            ...
            const description = await test.verifySuggestion({ arc }, 'Read from [fooValue: the-FOO] (foo-name) and populate [A list of foo with values: foo-1, foo-2].');
...
recipe
  ...
  A
    ...
    `));
            ...
            let description = await test.verifySuggestion({ arc }, 'Display X1-foo, create X1::X1-foo, and create X2::X2-foo.');
            ...
            description = await test.verifySuggestion({ arc, relevance }, 'Display X2-foo, create X2::X2-foo, and create X1::X1-foo.');
...
recipe
  ...
  B
    ...
    `));
            ...
            const description = await test.verifySuggestion({ arc }, 'Do A with b-foo (the-FOO), output B to b-foo, and output B to b-foo (another-FOO).');
            ...
            await test.verifySuggestion({ arc, relevance }, 'Do A with b-foo (the-FOO), output B to b-foo (another-FOO), and output B to b-foo.');
        });
    });
    tests.forEach((test) => {
        ...
        recipe
          ...
          P
           ...
           consume root as slot0`;
            ...
            assert.isTrue(recipe.isResolved());
            const arc = createTestArc(recipe, manifest);
            const tStore = await arc.createStore(myBESTType, undefined, 'test:1');
  ...
  HasDescription
    ...
    it('has no particles description', async () => {
        const verify = async (manifestStr, expectedDescription) => {
            ...
            assert.deepEqual(description.getRecipeSuggestion(DescriptionDomFormatter), { template: expectedDescription, model: {} });
        };
