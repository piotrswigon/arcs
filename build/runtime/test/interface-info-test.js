...
describe('interface', () => {
    ...
    it('Maybe ensure resolved does not mutate on failure', () => {
        const constrainedType1 = TypeChecker.processTypeList(TypeVariable.make('a'), [{ type: EntityType.make(['Thing'], {}), direction: 'in' }]);
        const constrainedType2 = TypeChecker.processTypeList(TypeVariable.make('b'), [{ type: EntityType.make(['Thing'], {}), direction: 'out' }]);
        const unconstrainedType = TypeVariable.make('c');
        const allTypes = [constrainedType1, constrainedType2, unconstrainedType];
        const allTypesIface = new InterfaceInfo('Test', [{ type: constrainedType1 }, { type: unconstrainedType }, { type: constrainedType2 }], []);
        ...
        assert.isTrue(allTypes.every(t => !t.isResolved()), 'Types should not have been modified by a failed maybeEnsureResolved()');
        const constrainedOnlyIface = new InterfaceInfo('Test', [{ type: constrainedType1 }, { type: constrainedType2 }], []);
        assert.isTrue(allTypes.every(t => !t.isResolved()));
...
//# sourceMappingURL=interface-info-test.js.map