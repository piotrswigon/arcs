...
recipe R3
  ...
  P2
    ...
    it('merges suggestions unchanged', async () => {
        ...
        const { helper, result1, result2 } = await prepareMerge(`${commonManifestStr}${recipeOneStr}${recipeTwoStr}`, `${commonManifestStr}${recipeOneStr}${recipeTwoStr}`);
    ...
    it('merges suggestions union', async () => {
        const { helper, result1, result2 } = await prepareMerge(`${commonManifestStr}${recipeOneStr}${recipeTwoStr}`, `${commonManifestStr}${recipeTwoStr}${recipeThreeStr}`);
        assert.lengthOf(result1.suggestions, 2);
...
recipe R4
  ...
  P1
    thing -> thing1Handle
    `;
        const { helper, result1, result2 } = await prepareMerge(`${commonManifestStr}${recipeOneStr}${recipeTwoStr}${recipeThreeStr}`, `${commonManifestStr}${recipeThreeStr}${recipeFourStr}`);
        assert.lengthOf(result1.suggestions, 3);
        assert.lengthOf(result2.suggestions, 2);
    ...
    it('merges all outdated suggestions', async () => {
        const { helper, result1, result2 } = await prepareMerge(`${commonManifestStr}${recipeTwoStr}`, `${commonManifestStr}${recipeThreeStr}`);
    ...
    it('merges same suggestion with older store versions', async () => {
        const { helper, result1, result2 } = await prepareMerge(`${commonManifestStr}${recipeTwoStr}`, `${commonManifestStr}${recipeTwoStr}${recipeThreeStr}`);
        assert.lengthOf(result1.suggestions, 1);
        assert.lengthOf(result2.suggestions, 2);
...
//# sourceMappingURL=planning-result-test.js.map