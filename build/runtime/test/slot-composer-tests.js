...
recipe
  ...
  C
            ...
            for (const consumer of slotContext.slotConsumers) {
                for (const [subId, content] of consumer.renderings) {
                    if (!result[subId])
                        result[subId] = [];
                    if (content.model)
                        result[subId].push(content.model.title);
                }
            }
            return result;
        };
    ...
    it('renders inner slots in transformations without intercepting', async () => {
        ...
        const rootSlotConsumer = slotComposer.consumers.find(consumer => consumer.consumeConn.name === 'root');
        await rootSlotConsumer.contentAvailable;
        const detailSlotConsumer = slotComposer.consumers.find(consumer => consumer.consumeConn.name === 'detail');
...
//# sourceMappingURL=slot-composer-tests.js.map