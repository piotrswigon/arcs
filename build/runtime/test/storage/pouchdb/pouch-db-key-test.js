...
describe('pouch-db-key', () => {
    ...
    describe('child keys', () => {
        ...
        it('childKeyForHandle creates a new PouchDbKey with id suffix', () => {
            ...
            assert.equal(childKey.toString(), 'pouchdb://localhost:8080/user/prefix/path/handles/99');
        });
        it('childKeyForArcInfo creates a new PouchDbKey with fixed suffix', () => {
            const childKey = remoteKey.childKeyForArcInfo();
            assert.equal(childKey.toString(), 'pouchdb://localhost:8080/user/prefix/path/arc-info');
        });
    });
});
//# sourceMappingURL=pouch-db-key-test.js.map