...
describe('schema', () => {
    ...
    `);
        ...
        assert.throws(() => {
            new Collections({ collection: new Set([new Reference({ id: 'test', storageKey: 'test' }, new ReferenceType(BarType), null)])
            });
    ...
    it('enforces rules when storing tuple types', async () => {
      ...
      schema Tuples
        ...
        assert.throws(() => { tuples.t2 = { a: 1 }; }, TypeError, 'Cannot set tuple t2 with non-array value');
        assert.throws(() => { new Tuples({ t1: ['foo'] }); }, TypeError, 'Length mismatch setting tuple t1');
        assert.throws(() => { tuples.t2 = ['url', {}, true, 3]; }, TypeError, 'Length mismatch setting tuple t2');
        assert.throws(() => { new Tuples({ t1: ['foo', '55'] }); }, TypeError, /Type mismatch setting field t1 .* at index 1/);
        assert.throws(() => { tuples.t2 = [12, {}, false]; }, TypeError, /Type mismatch setting field t2 .* at index 0/);
    ...
    it('field with a single parenthesised value is a tuple not a union', async () => {
      ...
      schema SingleValueTuple
        ...
        assert.throws(() => { svt.t = 78; }, TypeError, 'Cannot set tuple t with non-array value');
    });
    ...
    it('handles field type conflict in schema unions', async () => {
        ...
        assert.isNull(Schema.union(Person, Product), 'price fields of different types forbid an union');
    });
...
//# sourceMappingURL=schema-tests.js.map