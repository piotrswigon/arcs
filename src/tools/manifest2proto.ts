...
export async function serialize2proto(path: string): Promise<Uint8Array> {
  const manifest: Manifest = await Runtime.parseFile(path);

  if (manifest.errors.some(e => e.severity === ErrorSeverity.Error)) {
    throw new Error(`Problems found in manifest '${path}'`);
  }
