...
export class PlanGenerator {
  ...
  createStorageKey(handle: Handle): string {
    ...
    if (handle.fate === 'join') {
      // console.log(handle.joinedHandles.map(h => h.toString() + ' sk = ' + h.storageKey).join('\n'));
      const components = handle.joinedHandles;
      const joinSk = `join://${components.length}/${components.map(h => `{${h.storageKey.embedKey()}}`).join('/')}`;
